...
Der Platzhalter ``` plaintext<TenantID>``` muss in der nachfolgenden URL durch die Tenant ID aus Schritt 1 ersetzt werden. Anschließend kann die erstellte URL mit Tenant Admin (Global Administrator) Rechten aufgerufen werden. Die in der URL enthaltene ``` plaintext client_id ``` ist die ID von Apple Internet Accounts.
